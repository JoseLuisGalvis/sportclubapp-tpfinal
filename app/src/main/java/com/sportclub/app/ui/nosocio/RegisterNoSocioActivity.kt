...
import com.sportclub.app.databinding.ActivityRegisterNoSocioBinding
...
class RegisterNoSocioActivity : AppCompatActivity() {
    ...
    private lateinit var binding:     ActivityRegisterNoSocioBinding
    private lateinit var fotoManager: FotoManager

    private val dateFormatter = DateTimeFormatter.ofPattern("dd/MM/yyyy")
        .withZone(ZoneId.systemDefault())

    private val viewModel: RegisterNoSocioViewModel by viewModels()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding     = ActivityRegisterNoSocioBinding.inflate(layoutInflater)
        fotoManager = FotoManager(this)
        setContentView(binding.root)
        supportActionBar?.title = "Registro de No Socio"
    ...
    private fun setupListeners() {
        binding.btnFechaVisita.setOnClickListener {
    ...
    private fun observeUiState() {
        lifecycleScope.launch {
            viewModel.uiState.collectLatest { state ->
                ...
                binding.btnFechaVisita.text = dateFormatter.format(
                    Instant.ofEpochMilli(state.fechaVisitaMillis)
                )
    ...
    private fun observeEventos() {
        lifecycleScope.launch {
            viewModel.eventos.collect { evento ->
                when (evento) {
                    is RegisterNoSocioEvent.ShowError       -> toast(evento.message)
                    is RegisterNoSocioEvent.Success         -> mostrarDialogoExito(evento)
                    is RegisterNoSocioEvent.ReadyForPayment -> mostrarModalPago(evento)
                }
            }
        }
    }
    ...
    private fun mostrarDialogoFoto() {
        AlertDialog.Builder(this)
            .setTitle("Foto para pase diario")
            .setItems(arrayOf("Tomar foto con cámara", "Elegir de galería")) { _, which ->
                when (which) {
                    0 -> fotoManager.crearUriCamara("foto_visita")
    ...
    private fun mostrarDialogoExito(evento: RegisterNoSocioEvent.Success) {
        AlertDialog.Builder(this)
            .setTitle("No Socio registrado")
            .setMessage("Nombre: ${evento.nombre}\nDNI: ${evento.dni}\nFecha visita: ${evento.fechaVisita}")
            .setPositiveButton("Aceptar") { _, _ -> finish() }
            .show()
    }

    private fun mostrarModalPago(evento: RegisterNoSocioEvent.ReadyForPayment) {
        AlertDialog.Builder(this)
            .setTitle("¡Registro exitoso!")
            .setMessage(
                "Al presionar Aceptar se abrirá el\n" +
                        "módulo de pago para abonar la entrada diaria."
            )
            .setPositiveButton("Aceptar") { _, _ ->
                startActivity(
                    Intent(this, StripePaymentActivity::class.java).apply {
                        ...
                        putExtra(StripePaymentActivity.EXTRA_TYPE,        "nosocio")
                        putExtra(StripePaymentActivity.EXTRA_ENTITY_ID,   evento.idNoSocio)
                    }
                )
                finish()
            }
